...
const CONFIG = {
  ...
  counts: {
    foliage: 17000,
    ornaments: 400,   // 拍立得照片数量
    elements: 150,    // 圣诞元素数量
    lights: 1000       // 彩灯数量
  },
  tree: { height: 32, radius: 15 }, // 树体尺寸
  photos: {
...
const ChristmasElements = ({ state }: { state: 'CHAOS' | 'FORMED' }) => {
  ...
  return (
    <group ref={groupRef}>
      {data.map((obj, i) => {
        let geometry; if (obj.type === 0) geometry = boxGeometry; else if (obj.type === 1) geometry = sphereGeometry;else if (obj.type === 2) geometry = caneGeometry;
        else geometry = sphereGeometry; // 银色球也是球体 //else geometry = caneGeometry;

        return ( <mesh key={i} scale={[obj.scale, obj.scale, obj.scale]} geometry={geometry} rotation={obj.chaosRotation}>
          //<meshStandardMaterial color={obj.color} roughness={0.3} metalness={0.4} emissive={obj.color} emissiveIntensity={0.2}/>
          <meshStandardMaterial
            color={obj.color}
            roughness={obj.type === 3 ? 0.08 : 0.3}
            metalness={obj.type === 3 ? 1.0 : 0.4}
            emissive={obj.type === 3 ? '#000000' : obj.color}
            emissiveIntensity={obj.type === 3 ? 0.0 : 0.2}
          />
...
export default function GrandTreeApp() {
  ...
  useEffect(() => {
    ...
    audio.volume = 0.35; // 0~1
    if (musicOn) {
      audio.play().catch(() => {
        // 如果浏览器拦截了自动播放，这里不会报到 UI，上层用按钮触发一般就OK
      });
    } else {
      audio.pause();
    }
    ...
    <div style={{ width: '100vw', height: '100vh', backgroundColor: '#000', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', bottom: '30px', right: '40px', zIndex: 10, display: 'flex', gap: '10px' }}>
        ...
        <button
          ...
          style={{
            padding: '12px 15px',
            backgroundColor: musicOn ? '#FFD700' : 'rgba(0,0,0,0.5)',
            border: '1px solid #FFD700',
            color: musicOn ? '#000' : '#FFD700',
